    ...
    <div style="padding:30px;">
          ...
          <div style="text-align:left;margin-top:30px;">
            ...
            <el-table border :data="Tags" style="margin-top:30px;width:100%">
              ...
              <template slot-scope="scope">
                  <div class="colorBox" :style="{backgroundColor:scope.row.tagColor}"></div>
              </template>
      ...
      <el-dialog title="添加标签信息" :visible.sync="addTagVisible">
        <div style="text-align:left;">
          ...
          <p>标签别名</p>
          <el-input placeholder="请输入标签名" v-model="TagAlias"></el-input>
          <p>标签描述</p>
  ...
  methods: {
    ...
    confirmAddTag() {
      if (
        ...
        this.axios.post("/tag", body).then(res => {
          this.$message({
            showClose: true,
            message: "添加成功",
            type: "success"
    ...
    deleteTag(index, Tags) {
      this.$confirm("此操作将永久删除该数据, 是否继续?", "提示", {
        ...
        let id = this.Tags[index].tagId;
        this.axios.delete("/tag/" + id).then(res => {
          this.$message({
            showClose: true,
            message: "删除成功",
            type: "success"
    ...
    editTag(index, Tags) {
      ...
      let id = Tags[index].tagId;
      console.log(id);
      for (let item of this.Tags) {
        if (id == item.tagId) {
          this.TagName = item.tagName;
          this.TagAlias = item.tagAlias;
          this.TagDescription = item.tagDescription;
          this.color = item.tagColor;
    ...
    confirmModifyTag() {
      if (
        ...
        this.axios.put("/tag", body).then(res => {
          this.$message({
            showClose: true,
            message: "修改成功",
            type: "success"
